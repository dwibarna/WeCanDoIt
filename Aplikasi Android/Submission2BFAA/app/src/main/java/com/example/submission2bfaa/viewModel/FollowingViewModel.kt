...
class FollowingViewModel:ViewModel() {
        ...
        client.get(url,object:AsyncHttpResponseHandler(){
            override fun onSuccess(
                ...
                try {
                    ...
                    following.username = jsonObject.getString("login")
                    following.avatar = jsonObject.getString("avatar_url")
                    following.followers = jsonObject.getString("followers")
                    following.dataFollowing = jsonObject.getString("following")
                    listFollowingNonMutable.add(following)
                    listFollowing.postValue(listFollowingNonMutable)
                }catch (e:Exception){
                    e.printStackTrace()
                }
            }
    ...
    fun dataFollowing(person: String,context: Context){
        val client = AsyncHttpClient()
        val url = "https://api.github.com/users/$person/following"
        val token = "token 65d954dc1ba31e07beaf2bab17ec97a7cb96ff90"
        client.addHeader("Authorization", token)
        client.addHeader("User-Agent", "request")
        client.get(url, object : AsyncHttpResponseHandler() {

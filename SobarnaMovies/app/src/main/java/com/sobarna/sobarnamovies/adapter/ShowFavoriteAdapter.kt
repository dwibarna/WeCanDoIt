...
class ShowFavoriteAdapter : RecyclerView.Adapter<ShowFavoriteAdapter.CourseViewHolder>() {
    private val listCourses = ArrayList<ResultX>()

    fun setCourses(courses: List<ResultX>?) {
        ...
        fun bind(movie: ResultX){
            ...
            with(binding){
                Glide.with(itemView.context)
                    ...
                    .into(ivMovieList)
                tvTitleList.text = movie.name
                tvReleaseDateList.text = movie.first_air_date

                itemView.setOnClickListener {
                    val intent = Intent(itemView.context,DetailActivity::class.java)
                    intent.putExtra(DetailActivity.EXTRA_SHOW, movie.id)
